...
if(isset($_POST["btn"]))
{
	...
	{//notempty
		...
		if(!isset($pictmp))
		{
			// in fekr konam eshtebahe inike paeen ezafe kardam bayad doros bashe vali baz inaro bayad test koni
			//$sql1="UPDATE `product` SET `cat-id` = ?, `name` = ?, `gheymat` = ?, `tozihat` = ? ' WHERE `product`.`product-id` = ?;";
			$sql1="UPDATE `product` SET `cat-id` = ?, `name` = ?, `gheymat` = ?, `tozihat` = ? ' WHERE `product-id` = ?;";
			...
			if($query1)
			{
				// inja khodet edited o gozashti 1010 hala man too product-edit az in GET e estefade mikonam safaro refresh mikonam !
				// yani vaghti update anjam shod bere oon safhe in GET am ba khodesh bebare
				// bad oonvar migam age GET o (1010) o ba khodesh ovord safhe reload she .
				// baz mitooni baraye baghyasham benevsi yani masalan age am update nashod ye meghdari ba GET bebare bad oonvar error benevisi ya harchi hala !
